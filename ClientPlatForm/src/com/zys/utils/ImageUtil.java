...
public class ImageUtil {
	...
	public static Image cutImage(File file, int x, int y, int width, int height) {
		Image image = null;
		...
		try {
			...
			// 获取文件的后缀名
			String postFix = getPostfix(file.getAbsolutePath());
			...
			image = bi;
			// // 保存新图片
			// ImageIO.write(bi, postFix,
			// new File(subpath + "_" + new Date().getTime() + "."
			// + postFix));
		} catch (IOException e) {
			e.printStackTrace();
		} finally {
	...
	public static Iterator<ImageReader> getImageReadersByFormatName(
			String postFix) {
		if (IMAGE_FORM_OF_JPG.equals(postFix))
			return ImageIO.getImageReadersByFormatName(IMAGE_FORM_OF_JPG);
		else if (IMAGE_FORM_OF_PNG.equals(postFix))
